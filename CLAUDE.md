...
- Setting `voice_edit_hotkey` (default `Ctrl+F9`, empty = disabled) + `voice_edit_primary_lang` /
  ...
  `VoiceEditCard`)
- Flow: hotkey → `capture_selection_inner` (saves clipboard → clears it → simulated `Ctrl+C`
  via enigo → reads back → **always restores the clipboard**, and records the source `HWND`
  from `GetForegroundWindow`) → `voice-edit-open` event → overlay window shown → mic opened
...
- `voice_edit.rs` reuses the streaming `SpeechSegmenter` with its own profile
  ...
  `streaming.rs` is untouched. The recorder has a single tap slot, so Voice Edit is refused
  while a dictation is running. A 30 s timeout closes the mic if nothing is said
...
- `useVoiceEdit` (mounted once in `Dashboard`) owns everything needing settings/auth/cloud:
  eligibility gate (upsell **before** any network call), instruction transcription via
  `transcribeCloud`, LLM call via the **unchanged** `notes_assist_cloud`, then pushes
  `voice-edit-state` back to the overlay
- Replacement (`replace_selection`) does `SetForegroundWindow` + `type_text_at_cursor`.
  **Known risk**: Windows restricts `SetForegroundWindow`; if it fails, the text stays in the
  overlay and on the clipboard. Case 1 of the E2E checklist is the blocking validation
...
- Tables Supabase : `user_notes` (id, user_id, title, content_html, folder_id, favorite, order, created_at, updated_at, deleted_at + CHECK octet_length <= 3 MB + FK folder_id ON DELETE SET NULL), `user_folders` (id, user_id, name, order, created_at, updated_at, deleted_at) — migrations `20260601000800-001000`. Fonction quota `compute_user_sync_size_v2` inclut notes + folders actifs.
- UX : `src/components/notes/NoteSizeWarning.tsx` (banner amber >3 MB via TextEncoder bytecount) + `SyncActivationModal.tsx` (compteurs notes/folders, threshold non-trivial étendu) + `AccountSection.tsx::SyncedInventoryGrid` (5 cards : Préférences + Snippets + Dictionnaire + Notes + Dossiers) + `SyncPlanCard` (carte plan Free/Starter/Pro avec upsell free).
- Quotas freemium : Free 10 MB / Starter 100 MB / Pro 500 MB. Hard cap par note 3 MB tous plans (relevé de 1 → 3 MB le 2026-06-29, migration `20260629000000_user_notes_content_size_3mb`). Lecture plan via table `subscriptions` — `active` + `on_trial` actifs, sinon fallback free.
...
- **Partitionnement cloud par profil — Plan A (feat/sync-multi-profile, 2026-06-25)** : coupe nette — chaque profil local obtient un `cloud_profile_id` UUID (stocké dans `sync-meta.json` per-profile, via `ensureCloudProfileId` dans `src/lib/sync/cloud-profile.ts`, câblé par `SyncContext`). Nouvelle table Supabase `user_profiles` (id, user_id, name, created_at, updated_at, deleted_at — LWW + soft-delete comme `user_folders`) avec RLS stricte (`auth.uid() = user_id`). Colonne `profile_id` (FK → `user_profiles.id` ON DELETE CASCADE) ajoutée aux 5 tables sync : `user_settings` (PK → `(user_id, profile_id)`), `user_dictionary_words` (PK → `(user_id, profile_id, word)`), `user_snippets`, `user_notes`, `user_folders` ; index par profil ajoutés. RLS inchangé — `profile_id` est un discriminant intra-user, pas une frontière de sécurité. `sync-push` Edge Function v3 : stamp `profile_id` sur chaque upsert + gère les ops `profile-upsert` / `profile-delete`. `account-export` v3 : settings en array + table `user_profiles` incluse (`export_version: 3`) — résout le crash GDPR multi-profil. Quota (`compute_user_sync_size`) inchangé = par compte. Côté TS : `client.ts` scope `pullAll`/`pushOperations` par `profile_id` ; stores snippets/dictionnaire déplacés per-profile (commandes Rust `get_active_profile_snippets_path` / `get_active_profile_dictionary_path` → `snippets_store_path` / `dictionary_store_path` dans `profiles.rs`) + migration locale one-shot. Tests : 2 pgtap (`rls_user_profiles.sql`, `profile_id_columns.sql`), tests Rust unit (profil path helpers + migration), 40 Deno Edge (sync-push v3), Vitest (cloud-profile, client, schemas, stores). Plan B (suivi) : onboarding multi-appareils, UI suppression profil propagée, rework `AccountSection`, pull du registre `user_profiles` (`PullResult.profiles` vide en Plan A). Garde-fou connu (à durcir en Plan B) : `flushQueue` peut drainer des ops note/folder avant que le `profile-upsert` n'atterrisse pendant la fenêtre d'activation → violation FK transitoire → retry/DLQ, pas de perte. Plan : `docs/superpowers/plans/2026-06-25-sync-multi-profile-plan-a-foundation.md` ; spec : `docs/superpowers/specs/2026-06-25-sync-multi-profile-design.md`.
- **Fix push initial robuste + cap note (2026-06-26)** : root cause d'un sync « vert mais DB vide » → une note > 1 Mo (UTF-8) fait rejeter TOUT le batch (`PushBodySchema` Zod valide le body de façon atomique → 400 `invalid body`), erreur avalée par `enableSync`. Corrigé : (1) `src/lib/sync/note-size.ts` (`NOTE_SIZE_LIMIT_BYTES`, `noteContentBytes`, `isNoteSyncable` — byte-count UTF-8 = autorité DB `octet_length`) ; les notes hors-cap sont **sautées** du push (restent locales) au lieu d'empoisonner le batch — gardé dans `notes-store` (enqueue) ET `SyncContext.fullPush` ; comptées via `scanOversizedNoteCount` → `SyncState.oversized_note_count` → bannière `sync.oversizedNotes_warning` dans `AccountSection`. (2) Push initial extrait en `fullPush` **idempotent** + flag `initial_push_done` dans sync-meta : l'effet de mount le **re-tente** tant qu'il n'a pas réussi (offline / erreur transitoire / note trimmée), `syncNow` le rejoue aussi → plus besoin de toggle off/on pour récupérer. (3) `enableSync`/`fullPush` remontent le **vrai statut** (`error` + `last_error`) au lieu de toujours afficher `idle` ; inspection de `resp.results[].ok` (échecs per-op FK/RLS) en plus de `resp.ok`. `NoteSizeWarning.tsx` réutilise désormais le helper centralisé.
- **Import multi-appareils — Plan B partiel (feat/sync-multi-device-import, 2026-06-26)** : résout la régression « PC2 vierge ne voit aucun profil PC1 ». Cause : chaque appareil mintait un `cloud_profile_id` aléatoire et le pull était scopé dessus (`client.ts` `.eq("profile_id", …)`), donc un appareil ne pouvait jamais découvrir les partitions créées ailleurs (`PullResult.profiles` vide en Plan A). Livré : (1) `client.ts::pullProfilesRegistry()` lit le registre `user_profiles` account-wide (RLS SELECT `auth.uid() = user_id`, policy `user_profiles_select_own` déjà présente), drop+count des rows malformées. (2) `src/lib/sync/profile-import.ts` : `listLocalCloudBindings()` (map `cloud_profile_id → local id` en lisant chaque sync-meta), `listImportableCloudProfiles()` (registre cloud moins les déjà-liés), `importCloudProfile()` (crée un profil local, pré-écrit `cloud_profile_id`+`enabled=true`+`initial_push_done=false` dans son sync-meta, renvoie le nouvel id ; le caller `switch_profile` → reload → le mount lifecycle existant pull les données). (3) Commande Rust `get_profile_sync_meta_path(id)` (chemin sync-meta d'un profil **non-actif**, valide l'existence dans le manifest) dans `commands/profiles.rs` + enregistrée `lib.rs`. (4) UI `CloudProfilesImportCard` dans `AccountSection` (signed-in, rendu seulement s'il y a des importables) + i18n `sync.import.*`. Tests : Vitest `profile-import.test.ts` (6) + `client.test.ts` (pullProfilesRegistry, +2 ; helper `makeQuery` étendu `.is`). **Reste Plan B** : suppression de profil cloud propagée (les partitions « Default » junk créées par activation sur appareil vierge restent visibles comme importables), onboarding auto au login, garde-fou FK `flushQueue` pendant l'activation. Pas de migration DB (registre déjà créé en Plan A).
