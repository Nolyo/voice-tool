...
- **Partitionnement cloud par profil — Plan A (feat/sync-multi-profile, 2026-06-25)** : coupe nette — chaque profil local obtient un `cloud_profile_id` UUID (stocké dans `sync-meta.json` per-profile, via `get_or_create_cloud_profile_id` dans `SyncContext`). Nouvelle table Supabase `user_profiles` (id, user_id, name, created_at, updated_at) avec RLS stricte. Colonne `profile_id` (FK → `user_profiles.id` ON DELETE CASCADE) ajoutée aux 5 tables sync : `user_settings`, `user_dictionary_words`, `user_snippets`, `user_notes`, `user_folders` ; tous index et policies RLS mis à jour. `sync-push` Edge Function v3 : stamp `profile_id` sur chaque upsert + gère les ops `profile-upsert` / `profile-delete`. Côté TS : `client.ts` scope `pullAll` par `profile_id` ; stores snippets/dictionnaire déplacés per-profile (`getSnippetsStorePath` / `getDictionaryStorePath` dans `commands/profiles.rs`) — migration locale one-shot. Tests : 3 pgtap (`rls_user_profiles.sql`, `profile_id_columns.sql`, RLS cross-profil), 5 Rust unit (profil path helpers), 40 Deno Edge (sync-push v3). Plan B (suivi) : onboarding multi-appareils, UI suppression profil, `account-export` v3, rework `AccountSection`. Plan : `docs/superpowers/plans/2026-06-25-sync-multi-profile-plan-a-foundation.md`.
