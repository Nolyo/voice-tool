...
| **0016** — Notes sync strategy | 2 tables séparées notes + folders, LWW par item, soft-delete + purge 30j, hard cap 1 MB par note, quota freemium hybride (Free 10 MB / Starter 100 MB / Pro 500 MB) | ✅ | Migrations : `supabase/migrations/20260601000800_user_folders.sql`, `20260601000900_user_notes.sql`, `20260601001000_compute_user_sync_size_v2.sql`. Edge : `supabase/functions/sync-push/{schema,index}.ts` (4 nouvelles ops + per-plan quota), `account-export/index.ts` (export_version 2), `purge-account-deletions/index.ts` (étendu notes + folders). Backend Rust : `src-tauri/src/notes.rs` (soft-delete + purge_soft_deleted_notes_post_pull + import_note_for_backup), `src-tauri/src/folders.rs` (updated_at + soft-delete + purge + import). Frontend : `src/lib/sync/{types,schemas,mapping,client,merge,notes-store,folders-store,quota}.ts`, `src/hooks/{useNotes,useFolders}.ts` (debounce 2s sur updateNote), `src/contexts/SyncContext.tsx` (pull lifecycle notes + folders + purge post-pull). UX : `src/components/notes/NoteSizeWarning.tsx`, `SyncActivationModal.tsx` + `AccountSection.tsx` (compteurs + carte plan + upsell free). Tests : 266/266 Vitest + tests Rust + pgtap RLS + Deno Edge. Checklist E2E : `docs/v3/03-sync-notes-e2e-checklist.md`. |
| **0017** — Closure sub-épique 03-sync-notes | Ajustements vs spec + follow-ups identifiés | ✅ | Cf. `0017-sub-epic-03-closure.md`. |
