...
///
/// Phase A: kept for the Notes AI assistant (BYOK). Phase B will migrate this
/// caller to the managed cloud endpoint and remove this module.
pub async fn chat_completion(
    ...
    if api_key.trim().is_empty() {
        return Err(anyhow!("Missing OpenAI API key. Configure it in settings."));
    }
    ...
    tracing::info!(
        "Sending request to OpenAI Chat Completions API (model: {})...",
        DEFAULT_MODEL
    );
