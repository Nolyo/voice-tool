...
pub struct AuthState {
    ...
    /// Wrapped in Zeroizing so the buffer is wiped on drop instead of lingering
    /// in freed-but-not-overwritten heap pages.
...
pub fn store_refresh_token(
    ...
    match write_keyring(KEYRING_ACCOUNT_REFRESH_TOKEN, &token) {
        ...
        Err(e) => {
            ...
            auth.memory_fallback_refresh_token = Some(Zeroizing::new(token));
            Ok(KeyringStatus {
                available: false,
...
pub fn get_refresh_token(state: State<'_, AppState>) -> Result<Option<String>, String> {
    ...
    match read_keyring(KEYRING_ACCOUNT_REFRESH_TOKEN) {
        ...
        Ok(None) => {
            ...
            Ok(auth.memory_fallback_refresh_token.as_ref().map(|t| t.as_str().to_string()))
        }
        Err(e) => {
