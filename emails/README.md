...
3. When done, build: `pnpm email:build` → produces `./dist/<slug>.html`.
4. Run tests: `pnpm test emails/` → 23 tests should pass (4 tokens + 18 templates + 5 build).
5. Commit the `.tsx` AND the rebuilt `emails/dist/*.html` together (snapshot regression test enforces this).
...
- `build.tsx` — render all templates to `./dist/*.html` with Liquid placeholder safety
...
If you add a template with a NEW Liquid variable (e.g., `{{ .Email }}`), verify it survives the build by checking `emails/dist/<your-template>.html` for the literal placeholder. Add a corresponding assertion in the build test.
